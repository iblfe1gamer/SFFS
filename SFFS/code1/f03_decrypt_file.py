...
3. Check version byte (v1 or v2)
4. Extract original filename (v2) or reconstruct from .sffs name (v1)
5. Extract IV, auth_tag, hash_pre, original_size
6. Create AES-GCM cipher with IV
7. Attempt decryption (automatically verifies auth tag)
8. If auth tag invalid: delete any partial output, raise SecurityError
9. If successful: write decrypted plaintext to output_dir with original filename
10. Compute SHA-256 of decrypted output (hash_post)
11. Compare hash_post with hash_pre (via verifyHash module)
12. Return result dict with hash comparison
"""
...
def decryptFile(
    ...
    # Minimum header for v1: Magic (4) + Version (1) = 5 bytes
    ...
    if version == SFFS_V1:
        # v1 fixed header (77 bytes total):
        #   Magic(4) + Version(1) + IV(16) + AuthTag(16) + Hash(32) + Size(8) = 77
        HEADER_SIZE = 77
        original_name = sffs_path.stem  # Reconstruct from .sffs filename
        iv = file_content[pos : pos + 16]
        pos += 16
        auth_tag = file_content[pos : pos + 16]
        pos += 16
    ...
    elif version == SFFS_V2:
        # v2 variable header:
        #   Magic(4) + Version(1) + NameLen(2) + Name(N) + IV(16) + AuthTag(16) + Hash(32) + Size(8)
        name_len = struct.unpack("H", file_content[pos : pos + 2])[0]
        pos += 2
        original_name = file_content[pos : pos + name_len].decode("utf-8")
        pos += name_len
        HEADER_SIZE = 7 + name_len + 16 + 16 + 32 + 8  # = 79 + name_len
        iv = file_content[pos : pos + 16]
        pos += 16
        auth_tag = file_content[pos : pos + 16]
        pos += 16
        ...
        raise ValueError(f"Unknown SFFS version: {version}")

    # Ciphertext is everything after the header
    ciphertext = file_content[pos:]

    # Verify we have enough data (header + at least some ciphertext)
    if len(file_content) < HEADER_SIZE:
        raise ValueError("SFFS file too small — possibly corrupted")
