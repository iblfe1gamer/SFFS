...
def verifyFileIntegrity(original_path: Path, decrypted_path: Path) -> dict:
    ...
    if str(original_path).endswith(".sffs"):
        ...
        with open(original_path, "rb") as f:
            # Skip magic (4) + version (1) + IV (16) + auth_tag (16)
            f.read(4 + 1 + 16 + 16)
            # Read hash (32 bytes)
            hash_pre_bytes = f.read(32)
            hash_pre = hash_pre_bytes.hex()
            # Read remaining (file_size + ciphertext)
            f.read(8)  # file_size

        # Compute hash of decrypted file
        with open(decrypted_path, "rb") as f:
            hash_post = sha256(f.read()).hexdigest()

    # Handle plain file comparison
    elif original_path.exists():
        with open(original_path, "rb") as f:
            original_hash = sha256(f.read()).hexdigest()

        with open(decrypted_path, "rb") as f:
            hash_post = sha256(f.read()).hexdigest()

        hash_pre = original_hash
