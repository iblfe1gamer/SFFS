...
│  Magic (4 bytes):    "SFFS"                                      │
│  Version (1 byte):   0x02                                       │
│  Name Len (2 bytes): Original filename length (uint16 LE)       │
│  Name (N bytes):     Original filename including extension      │
...
Total header = 79 + N bytes before ciphertext (N = filename length).
...
def encryptFile(
    ...
    original_name = input_path.name.encode("utf-8")

    # Assemble SFFS file header
    # Header fields:
    #   Magic (4 bytes): Magic number "SFFS"
    #   Version (1 byte): Format version
    #   Name Len (2 bytes): Length of original filename (uint16 LE)
    #   Name (N bytes): Original filename including extension
    ...
    header = (
        ...
        struct.pack("H", len(original_name)) +       # Name length (2 bytes)
        original_name +                              # Original filename (N bytes)
        iv +                                         # IV (16 bytes)
