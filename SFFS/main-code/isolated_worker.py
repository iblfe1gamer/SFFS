...
# When frozen (PyInstaller), find and mount the hidden SFFS partition for keystore access
if getattr(sys, "frozen", False) and not _env_root:
    try:
        from hidden_volume import find_sffs_partition, mount_hidden_volume as _mount_hv
        _vol = find_sffs_partition()
        if _vol:
            _mount_letter = os.environ.get("SFFS_MOUNT_LETTER", "S")
            if _mount_hv(_vol, _mount_letter):
                _ROOT = Path(f"{_mount_letter}:\\")
                os.environ["SFFS_ROOT"] = str(_ROOT)
    except Exception:
        pass
