...
def retrieveKey(keystore_path: Path, master_password: str) -> bytes:
    ...
    try:
        ...
        raise ValueError(
            "Keystore decryption failed — wrong password or corrupted keystore"
        ) from exc
...
if __name__ == "__main__":
    ...
    # Generate a fake RSA private key using PyCryptodome API
    from Crypto.PublicKey import RSA
    fake_key = RSA.generate(2048)
    fake_key_bytes = fake_key.export_key()  # PyCryptodome: returns PKCS1 PEM bytes
