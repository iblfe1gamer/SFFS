...
- hashlib.blake2b is stdlib since Python 3.6 — no external dependency needed
- We support it via hashlib directly for performance
...
def generateHash(target: Union[Path, bytes, str], algorithm: str = "sha256") -> str:
    """
    Generate a cryptographic hash of the given data.

    Args:
        target: A Path, str (file path), or raw bytes to hash
    ...
    Raises:
        ...
        TypeError: If target is neither Path, str, nor bytes
    """
    # Normalize str paths to Path objects so callers don't need to import Path
